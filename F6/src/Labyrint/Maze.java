...
public class Maze {
	...
	public Maze(){
		try{
			BufferedReader in = new BufferedReader(new FileReader("C:\\GitHub\\Algoritmer och Datastrukturer\\F6\\src\\Labyrint.txt"));
	...
	private boolean solve(Position p){
		...
		if (mazeMatrix[p.row][p.column].equals(mazeMatrix[goal.row][goal.column])) {
			return true;
		} else {
